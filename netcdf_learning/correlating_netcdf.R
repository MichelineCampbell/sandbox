...
### Any missing values in a cell will fuck up correlations/lm/mean/whatever (result in NA), so consider sub-settng. Only SEA has coverage for the whole period of GISTEMP
...
# data --------------------------------------------------------------------
sydtemp <- read.csv("netcdf_learning/sydney_temp/IDCJAC0002_066062_Data1.csv")
nc_data <- nc_open("D:/CampbellM1/R_learning_space/gisstemp_2020/gistemp250_GHCNv4.nc")

sydtemp <- clean_names(sydtemp)
...
fun <- function(x) {
  cor(x, sydtemp$mean_maximum_temperature_a_c)
}
fun <- function(x) {
  mean(x, na.rm = TRUE)
}
